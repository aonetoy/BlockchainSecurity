...
//			functionDefinitionINFO.put("kind", functionDefinition.getKind()); // ver. 0.5.1
	...
	public Map<String, String> getAllFunctionDefinitionsWithPublicVisibility() {
		Map<String, String> getAllFunctionDefinitionsWithPublicVisibility = new HashMap<String, String>();
		
		for(int i=0; i<getAllFunctionDefinitions().size(); i++) {
			String name = getAllFunctionDefinitions().get(i).getVisibility();
			String src = getAllFunctionDefinitions().get(i).getSrc();
			if(name.equals("public")) {
				getAllFunctionDefinitionsWithPublicVisibility.put(src, name);
			}
		}
		return getAllFunctionDefinitionsWithPublicVisibility;
	}
